...
## Setup (manual and explicit)

This vendored Codex copy contains the skill and reference playbooks only. The
upstream `scripts/setup.sh` is intentionally excluded because it downloads
`twozero.tox`, edits Hermes configuration, and enables a localhost control
server. Do not run that setup automatically. If twozero MCP is explicitly
requested later, review its current installer and verify the downloaded asset
...
3. **Restart the agent session** to pick up the MCP connection
...
| `scripts/setup.sh` | Not vendored; inspect upstream before any explicit setup |
